...
public class UserService implements IUserService {
	...
	public Object addCMSUser(CMSUserDTO cmsUserDTO) {
		...
		log.info("{} [{}][Add CMS User]: {} added new CMS User [username: {}], [userMail: {}], [userRole: {}]",
				...
				user.getUserName(), user.getUserMail(), user.getRoles());
	...
	public Object deleteUser(int id) {
		...
		log.info("{} [{}][delete User]: {} deleted user of id {}", new java.util.Date(),
				RequestContextHolder.currentRequestAttributes().getSessionId(), PrincipalUtils
					.getPrincipalUserName(SecurityContextHolder.getContext().getAuthentication().getPrincipal()),
				id);
		return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_2001.getValue());
	}
	...
	public Object logout() {
		String username = PrincipalUtils
			.getPrincipalUserName(SecurityContextHolder.getContext().getAuthentication().getPrincipal());
		log.info("{} [{}][Logout]: {} logged out from service", new java.util.Date(),
				RequestContextHolder.currentRequestAttributes().getSessionId(), username);
		httpSession.invalidate();
		return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_2001.getValue());
	}
		...
		if (configurationFlags.isLogin()) {
			...
			if (user == null)
				...
				if (passwordEncoder.matches(password, user.getUserPassword())) {
					...
					log.info("{} [{}][Login]: {} logged into service", new java.util.Date(),
							RequestContextHolder.currentRequestAttributes().getSessionId(), currentPrincipalName);
					return user;
				}
				else {
	...
	public Object changePassword(String oldPassword, String newPassword) {
		String username = PrincipalUtils
			.getPrincipalUserName(SecurityContextHolder.getContext().getAuthentication().getPrincipal());
		Object validateRequest = changePasswordRequestValidator.validateChangePassword(oldPassword, newPassword,
				username);
		if (validateRequest != null)
			return validateRequest;
		...
		if (passwordEncoder.matches(oldPassword, user.getUserPassword())) {
			...
			UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
					user.getUserName(), user.getUserPassword(), nowAuthorities);
			SecurityContextHolder.getContext().setAuthentication(authentication);
			log.info("{} [{}][Change password]: User {} changed password", new java.util.Date(),
					RequestContextHolder.currentRequestAttributes().getSessionId(), username);
			return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_2001.getValue());
		}
	...
	public Object editUserMail(String newUserMail) {
		...
		if (isNewMailUsedByAnyUser == null) {
			User user = userRepository.findByUserName(username).setUserMail(newUserMail);
			userRepository.save(user);
			log.info("{} [{}][Change mail]: User {} changed mail", new java.util.Date(),
					RequestContextHolder.currentRequestAttributes().getSessionId(), username);
			return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_2001.getValue());
		}
		return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_3011.getValue());
	...
	public Object editUserName(String newUsername) {
		...
		if (checkUser == null) {
			User user = userRepository.findByUserName(username).setUserName(newUsername);
			userRepository.save(user);
			Collection<SimpleGrantedAuthority> nowAuthorities = (Collection<SimpleGrantedAuthority>) SecurityContextHolder
				.getContext()
				.getAuthentication()
			...
			UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
					user.getUserName(), user.getUserPassword(), nowAuthorities);
			SecurityContextHolder.getContext().setAuthentication(authentication);
			log.info("{} [{}][Change username]: User {} changed username", new java.util.Date(),
					RequestContextHolder.currentRequestAttributes().getSessionId(), username);
			return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_2001.getValue());
		}
		return ErrorProvidedDataHandlerUtils.getErrorProvidedDataHandler(Code.CODE_3013.getValue());
	...
	public List<User> findCmsUsers() {
		Collection<String> roles = Arrays.asList(RoleName.ROLE_ADMIN.getRoleName(),
				RoleName.ROLE_MODERATOR.getRoleName(), RoleName.ROLE_EDITOR.getRoleName());
		List<User> mergedList = new ArrayList<>();
		roles.forEach(role -> mergedList.addAll(userRepository.findUserByRoles(roleRepository.findByName(role))));
		return mergedList;
	}
	...
	public List<CustomTopCommentersClass> findTheBestCommenter(int size) {
		...
		for (CommentsCountModel commentsCountModel : commentsCountModels) {
			customTopCommentersClassList.add(new CustomTopCommentersClass(
					Objects.requireNonNull(userRepository.findById(commentsCountModel.getCommentId()).orElse(null))
						.getUserName(),
					commentsCountModel.getTotal()));
		}
